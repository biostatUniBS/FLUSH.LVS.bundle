...
static int lvs_Choleski_decompose(double *X, double *L, int n, int lapack){
  int i,j,error_code;
  char upper = 'U';
...
/***********************************************************************
 **
 ** int lvs_Choleski_2_inverse(double *X, double *Xinv, int n)
 **
...
static int lvs_Choleski_2_inverse(double *X, double *Xinv, int n,int upperonly, int lapack){
...
/***********************************************************************
 **
 ** int lvs_Choleski_inverse(double *X, double *Xinv, double *work, int n)
 **
 ** double *X - matrix containing choleski decomposition in upper triangle
 ** double *Xinv - on output will contain the inverse
...
int lvs_Choleski_inverse(double *X, double *Xinv, double *work, int n, int upperonly){

  int error_code;
  
  error_code = lvs_Choleski_decompose(X, work, n,use_lapack);
  if (!error_code){
    error_code = lvs_Choleski_2_inverse(work, Xinv, n,upperonly,use_lapack);
  }
  return error_code;
...
static int lvs_SVD_compute(double *X, int n, double *s, double *u, double *v,int lapack){
...
/***************************************************************
 **
 ** int lvs_SVD_2_inverse(double *Xinv, int n, double *s, double *u, double *v,int lapack)
 **
 ** double *Xinv - on exit contains the inverse
 ** int n - Xinv is n by n
 ** double *s - lvs_SVD components length n
 ** double *u - lvs_SVD components n by n
 ** double *v - lvs_SVD components n by n
...
static int lvs_SVD_2_inverse(double *Xinv, int n, double *s, double *u, double *v,int lapack){
...
/***************************************************************
 **
 ** int lvs_SVD_inverse(double *X, double *Xinv, int n)
 **
 ...
 **
 ** Note that  we will use linpack routine for lvs_SVD at some point this will
...
int lvs_SVD_inverse(double *X, double *Xinv, int n){
  ...
  error_code = lvs_SVD_compute(X, n, s, u, v,use_lapack);
  lvs_SVD_2_inverse(Xinv,n, s, u, v,use_lapack);
...
/* void R_lvs_SVD_compute(double *X, int *n, double *s, double *u, double *v){
  lvs_SVD_compute(X, *n,s,u, v);
  } */


void R_lvs_SVD_inverse(double *X, double *Xinv, int *n){
  lvs_SVD_inverse(X, Xinv,*n);
}
